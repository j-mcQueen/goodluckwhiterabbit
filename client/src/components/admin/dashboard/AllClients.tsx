...
export default function AllClients({ ...props }) {
  ...
  const TableRow = ({ ...props }) => {
    ...
    return mobile ? (
      <tr
        key={client.code}
        className={`text-white border-t ${index < clients.length - 1 ? "border-b" : ""} border-solid border-white/20 text-base`}
      >
      ...
      <tr
        key={client.code}
        className={`text-white border-t ${index < clients.length - 1 ? "border-b" : ""} border-solid border-white/20 text-xl`}
      >
